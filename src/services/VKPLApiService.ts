...
export class VKPLApiService
{
      ...
      public static async getUserCurrent(authToken: string): Promise<APITypes.TTokenResponse>
      {
            const headers = { Authorization: `Bearer ${authToken}` };
            return await VKPLApiService.makeRequest<APITypes.TTokenResponse>("https://api.vkplay.live/v1/user/current", "GET", headers);
      }
      ...
      private static async makeRequest<T>(url: string, method: HTTPMethod, headers?: HeadersInit, params?: URLSearchParams, body?: string): Promise<T>
      {
            ...
            if (!res.ok)
                  throw new Error(`[api] Error in request: ${JSON.stringify({url: `${url}?${params}`, body: await res.json()}, null, 5)}`);
